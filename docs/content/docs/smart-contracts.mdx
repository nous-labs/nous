...
All contract addresses are exported via `QUBIC_CONTRACT_ADDRESSES` from `@nouslabs/sdk` in case you need to build raw transactions.

## Helper shortcuts

Every entry in `QUBIC_CONTRACTS` has a matching `query*` helper exported from `@nouslabs/sdk` to keep call sites terse:
...
```typescript
import { createQuery, parseResponse, QUBIC_CONTRACTS } from '@nouslabs/sdk';
...
```typescript
import { queryQX, queryQearn, parseResponse } from '@nouslabs/sdk';
...
```typescript
import { parseResponse } from '@nouslabs/sdk';
...
```typescript
import { QUBIC_CONTRACTS, createQuery } from '@nouslabs/sdk';
...
```typescript
import { queryQearn, parseResponse } from '@nouslabs/sdk';
...
```typescript
import { queryRandom, parseResponse } from '@nouslabs/sdk';
...
```typescript
import { queryCCF, parseResponse } from '@nouslabs/sdk';
...
import { QUBIC_CONTRACTS } from '@nouslabs/sdk';
const query = createQuery(QUBIC_CONTRACTS.QX, 1);
