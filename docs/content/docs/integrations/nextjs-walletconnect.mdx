...
<Tabs items={['npm', 'yarn', 'bun', 'pnpm']}>
  <Tab value="npm">
    ```bash
    npm install @nouslabs/sdk @walletconnect/sign-client @walletconnect/types
    ```
  </Tab>
  <Tab value="yarn">
    ```bash
    yarn add @nouslabs/sdk @walletconnect/sign-client @walletconnect/types
    ```
  </Tab>
  <Tab value="bun">
    ```bash
    bun add @nouslabs/sdk @walletconnect/sign-client @walletconnect/types
    ```
  </Tab>
  <Tab value="pnpm">
    ```bash
    pnpm add @nouslabs/sdk @walletconnect/sign-client @walletconnect/types
    ```
  </Tab>
</Tabs>
...
import { WalletConnectProvider } from '@nouslabs/sdk/react';
...
import { useWalletConnect } from '@nouslabs/sdk/react';
...
<Callout type="success">
  **Automatic Updates**: The provider automatically updates `accounts` and `isConnected` when users approve the connection. A one-time sync check happens 1.5 seconds after approval to ensure reliable state. No manual polling or state management needed!
</Callout>
...
In rare cases where you need to manually sync accounts (e.g., user switches accounts in wallet):

```tsx
import { useWalletConnect } from '@nouslabs/sdk/react';

export function AccountRefresher() {
  const { refreshAccounts, isConnected } = useWalletConnect();
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      console.log('Accounts refreshed:', accounts);
    } catch (error) {
  ...
  return (
    <button onClick={handleRefresh} disabled={!isConnected || isRefreshing}>
...
<Callout type="info">
  **When to Refresh**: The provider automatically syncs accounts after connection. Only use `refreshAccounts()` if you suspect the state is out of sync (very rare).
</Callout>
...
import { QubicQueryProvider, WalletConnectProvider } from '@nouslabs/sdk/react';
...
import { useState } from 'react';
import { useWalletConnect } from '@nouslabs/sdk/react';
import { useWCSignAndBroadcast, useBalance } from '@nouslabs/sdk/react/query';
...
```tsx
import { useWCSignAndBroadcast } from '@nouslabs/sdk/react/query';
...
```tsx title="app/layout.tsx"
import { QubicProvider, QubicQueryProvider, WalletConnectProvider } from '@nouslabs/sdk/react';
...
```tsx
import { useQubic, useWalletConnect } from '@nouslabs/sdk/react';
