...
export function ConnectButton() {
  const { adapter, isReady, accounts, isConnected } = useWalletConnect();
  ...
  const handleConnect = async () => {
    ...
    try {
      // adapter.connect() returns WalletConnectConnection - fully typed!
...
<Callout type="info">
  **State Management**: The provider tracks `accounts` and `isConnected` automatically. The `accounts` state updates when users approve the connection!
</Callout>
...
interface WalletConnectContextValue {
  ...
  accounts: WalletAccount[];             // Connected accounts
...
export function SendTransaction() {
  const { adapter, accounts } = useWalletConnect();
...
export default function HomePage() {
  const { adapter, accounts, isConnected, isReady } = useWalletConnect();
  const [recipient, setRecipient] = useState('');
  const [amount, setAmount] = useState('');
